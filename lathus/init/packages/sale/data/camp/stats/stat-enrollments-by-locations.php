...
[$params, $providers] = eQual::announce([
    ...
    'params'        => [
        ...
        'location' => [
            'type'              => 'string',
            'description'       => "The location where the child is hosted."
        ],
        'qty' => [
            'type'              => 'integer',
            'description'       => "Quantity of enrollments of location."
        ]
    ],
    'response'      => [
...
$map_center_locations_enrollments_qty = [];

foreach($camps as $camp) {
    if(!isset($map_center_locations_enrollments_qty[$camp['center_id']])) {
        $map_center_locations_enrollments_qty[$camp['center_id']] = [];
    }
    ...
    foreach($camp['enrollments_ids'] as $enrollment) {
        ...
        if(!isset($map_center_locations_enrollments_qty[$camp['center_id']][$camp['location']])) {
            $map_center_locations_enrollments_qty[$camp['center_id']][$camp['location']] = 0;
        }

        $map_center_locations_enrollments_qty[$camp['center_id']][$camp['location']]++;
    }
}

$center_ids = array_keys($map_center_locations_enrollments_qty);
...
foreach($map_center_locations_enrollments_qty as $center_id => $map_locations_qty) {
    ...
    foreach($map_locations_qty as $location => $qty) {
        $result[] = [
            'center'    => $center,
