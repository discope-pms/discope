    ...
    foreach($template['parts_ids'] as $part_id => $part) {
        ...
        elseif($part['name'] == 'service') {
            ...
            if(!$booking_options['has_bed_linens']) {
                $service_beds = $values['i18n']['not_bed_linens'];
            }
            else {
                if($booking_options['has_make_beds']) {
                    $service_beds = $values['i18n']['make_beds'];
                }
                else {
                    $service_beds = $values['i18n']['bed_linens'];
                }
            }

            $service_transport_roundtrip = ($has_roundtrip_transport) ? $values['i18n']['round_trip_transport'] : '';
            $service_transport_activities = ($has_activities_transport) ? $values['i18n']['activities_transport'] : '';
