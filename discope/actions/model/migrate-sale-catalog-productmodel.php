...
[$params, $providers] = eQual::announce([
    'description'   => 'Migrate the model discriminator for sale_catalog_productmodel.',
    'params'        => [
        'confirm' => [
...
$table = 'sale_catalog_productmodel';
...
$classify = static function (array $row) use($pos_model_ids) : string {
    $is_camp = !empty($row['is_camp']);
    $is_supply = !empty($row['is_supply']);
    $is_pos = isset($pos_model_ids[(int) $row['id']]);
    if((int) $is_camp + (int) $is_supply + (int) $is_pos > 1) {
        throw new Exception('conflicting_product_model_roles', EQ_ERROR_INVALID_CONFIG);
    }
    if($is_camp) {
        return 'sale\\camp\\catalog\\ProductModel';
    }
    if($is_supply) {
        return 'sale\\catalog\\Supply';
    }
    if($is_pos) {
        return 'sale\\catalog\\PosProductModel';
    }
    return 'sale\\catalog\\ProductModel';
};
