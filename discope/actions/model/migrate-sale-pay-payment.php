...
$classify = static function (array $row) : string {
    if(!empty($row['booking_id']) && !empty($row['enrollment_id'])) {
        throw new Exception('conflicting_payment_owner', EQ_ERROR_INVALID_CONFIG);
    }
    if(!empty($row['booking_id']) || !empty($row['order_payment_id'])) {
        return 'sale\\booking\\Payment';
    }
    return 'sale\\pay\\Payment';
};
