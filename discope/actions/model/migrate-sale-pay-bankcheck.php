...
[$params, $providers] = eQual::announce([
    'description'   => 'Migrate the model discriminator for sale_pay_bankcheck.',
    'params'        => [
        'confirm' => [
...
$required_columns = ['id', 'model', 'booking_id', 'enrollment_id'];
...
// Resolve every row from the mutually exclusive owners observed in the Lathus database.
$classify = static function (array $row) : string {
    $has_booking = !empty($row['booking_id']);
    $has_enrollment = !empty($row['enrollment_id']);

    if($has_booking && $has_enrollment) {
        throw new Exception('conflicting_bank_check_owners', EQ_ERROR_INVALID_CONFIG);
    }
    if($has_booking) {
        return 'sale\\booking\\BankCheck';
    }
    if($has_enrollment) {
        return 'sale\\pay\\BankCheck';
    }

    throw new Exception('unclassified_bank_check', EQ_ERROR_INVALID_CONFIG);
};
