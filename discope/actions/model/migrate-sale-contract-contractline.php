...
[$params, $providers] = eQual::announce([
    'description'   => 'Migrate the model discriminator for sale_contract_contractline.',
    'params'        => [
        'confirm' => [
...
$table = 'sale_contract_contractline';
...
$classify = static function (array $row) use($contracts) : string {
    ...
    if(!$id || !isset($contracts[$id])) {
        // TODO: check -> throw new Exception('orphan_contract_line', EQ_ERROR_INVALID_CONFIG);
    }
    return !empty($contracts[$id]['booking_id']) ? 'sale\\booking\\ContractLine' : 'sale\\contract\\ContractLine';
};
