...
[$params, $providers] = eQual::announce([
    'description'   => 'Migrate the model discriminator for finance_accounting_invoicelinegroup.',
    'params'        => [
        'confirm' => [
...
$table = 'finance_accounting_invoicelinegroup';
...
$invoices = $loadRows('finance_accounting_invoice', ['id', 'booking_id']);

// Resolve every row to one concrete ORM model before opening a transaction.
$classify = static function (array $row) use($invoices) : string {
    $id = (int) ($row['invoice_id'] ?? 0);
    if(!$id || !isset($invoices[$id])) {
        // TODO: handle -> throw new Exception('orphan_invoice_line_group', EQ_ERROR_INVALID_CONFIG);
    }
    if(!empty($invoices[$id]['booking_id'])) {
        return 'sale\\booking\\InvoiceLineGroup';
    }
    return 'finance\\accounting\\InvoiceLineGroup';
};
