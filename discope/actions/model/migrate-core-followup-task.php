...
$classify = static function (array $row) : string {
    if(!empty($row['booking_id']) || ($row['entity'] ?? null) === 'sale\\booking\\Booking') {
        return 'sale\\booking\\followup\\Task';
    }
    elseif(!empty($row['enrollment_id']) || ($row['entity'] ?? null) === 'sale\\camp\\Enrollment') {
        return 'sale\\camp\\followup\\Task';
    }
    return 'core\\followup\\Task';
};
