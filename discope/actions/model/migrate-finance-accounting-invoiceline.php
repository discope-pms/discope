...
[$params, $providers] = eQual::announce([
    'description'   => 'Migrate the model discriminator for finance_accounting_invoiceline.',
    'params'        => [
        'confirm' => [
...
$table = 'finance_accounting_invoiceline';
...
$classify = static function (array $row) use($invoices) : string {
    ...
    if(!$id || !isset($invoices[$id])) {
        // TODO: handle -> throw new Exception('orphan_invoice_line', EQ_ERROR_INVALID_CONFIG);
    }
    return !empty($invoices[$id]['booking_id']) ? 'sale\\booking\\InvoiceLine' : 'finance\\accounting\\InvoiceLine';
};
