...
[$params, $providers] = eQual::announce([
    'description'   => 'Migrate the model discriminator for sale_price_price.',
    'params'        => [
        'confirm' => [
...
$loadRows = static function (string $related_table, array $related_fields, bool $indexed = true) use($db) : array {
...
$products = $loadRows('sale_catalog_product', ['id', 'product_model_id', 'is_camp']);

$pos_category_ids = [];
foreach($loadRows('sale_catalog_category', ['id', 'code'], false) as $category) {
...
$pos_model_ids = [];
foreach($loadRows('sale_product_rel_productmodel_category', ['productmodel_id', 'category_id'], false) as $relation) {
...
$classify = static function (array $row) use($products, $pos_model_ids) : string {
    $id = (int) ($row['product_id'] ?? 0);
    if(!$id || !isset($products[$id])) {
        // TODO: handle throw new Exception('orphan_price_product', EQ_ERROR_INVALID_CONFIG);
    }
    $product = $products[$id];
    if(!empty($product['is_camp'])) {
        return 'sale\\camp\\price\\Price';
    }
    if(isset($pos_model_ids[(int) ($product['product_model_id'] ?? 0)])) {
        return 'sale\\catalog\\PosPrice';
    }
