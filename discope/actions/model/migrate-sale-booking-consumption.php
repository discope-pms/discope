...
[$params, $providers] = eQual::announce([
    'description'   => 'Migrate the model discriminator for sale_booking_consumption.',
    'params'        => [
        'confirm' => [
...
$table = 'sale_booking_consumption';
...
$classify = static function (array $row) : string {
    if(!empty($row['repairing_id']) && ($row['type'] ?? null) !== 'ooo') {
        throw new Exception('incoherent_repair_type', EQ_ERROR_INVALID_CONFIG);
    }
    return !empty($row['repairing_id']) ? 'sale\\booking\\Repair' : 'sale\\booking\\Consumption';
};
