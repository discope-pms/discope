...
// Load and validate the related invoices used for classification.
...
$classify = static function (array $row) use($invoices) : string {
    ...
    if(!$invoice_id) {
        return 'finance\\accounting\\AccountingEntry';
    }

    if(!isset($invoices[$invoice_id])) {
        // TODO: check -> throw new Exception('orphan_accounting_entry_invoice', EQ_ERROR_INVALID_CONFIG);
    }

    if(isset($invoices[$invoice_id]['booking_id'])) {
        return 'sale\\booking\\AccountingEntry';
    }
