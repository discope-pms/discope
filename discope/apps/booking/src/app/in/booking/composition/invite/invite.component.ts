...
@Component({
  selector: 'booking-composition-invite',
  templateUrl: 'invite.component.html',
  styleUrls: ['invite.component.scss']
})
export class BookingCompositionInviteComponent implements OnInit, AfterContentInit {
     ...
     *
     * We should received invoice.mail.subject, invoice.mail.body + organisation signature
     */
    private async fetchTemplates() {
        ...
        try {
            ...
            if(templates && templates.length) {
                ...
                const parts = await this.api.collect("communication\\TemplatePart",
                        ['id', 'in', template['parts_ids']],
                        ['name', 'value'], 'id', 'asc', 0, 10, this.lang
                    );
    ...
    public async onSend() {
        ...
        */
        ...
        try {
            this.loading = true;
            const response:any = await this.api.call('?do=sale_booking_guests_list_invite', {
                id: this.booking_id,
            ...
            this.is_sent = true;
            this.snack.open("Invitation envoyée avec succès.");
            this.loading = false;
        }
        catch(response:any) {
    ...
    public onclickBooking() {
        ...
        // prevent angular life cycles while a context is open
        this.cd.detach();
