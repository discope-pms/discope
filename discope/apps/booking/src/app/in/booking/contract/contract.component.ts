...
export class BookingContractComponent implements OnInit, AfterContentInit {
    ...
    public roomPlans: boolean = true;
    public activitiesPlanning: boolean = false;
    ...
    public ngAfterContentInit() {
        ...
        this.vm.activitiesPlanning.formControl.valueChanges.subscribe( (activitiesPlanning:boolean) => this.activitiesPlanning = activitiesPlanning);
    }
    ...
    private async fetchTemplates() {
        ...
        try {
            ...
            if(templates && templates.length) {
                ...
                const attachments = await this.api.collect("communication\\TemplateAttachment", [['id', 'in', template['attachments_ids']], ['attachment_type', 'in', attachment_types], ['lang_id', '=', this.lang_id]], ['name', 'document_id.name', 'document_id.hash'], 'id', 'asc', 0, 20, this.lang);
    ...
    private async loadCenter() {
        const result = <Array<any>> await this.api.read("identity\\Center", [this.booking.center_id], Object.getOwnPropertyNames(new Center()));
        if(result && result.length) {
            const item:any = result[0];
    ...
    private async loadCenterOffice() {
        const result = <Array<any>> await this.api.read("identity\\CenterOffice", [this.center.center_office_id], Object.getOwnPropertyNames(new CenterOffice()));
        if(result && result.length) {
            const item:any = result[0];
    ...
    public async onSend() {
        ...
        */
        ...
        try {
            this.loading = true;
            const response:any = await this.api.call('?do=sale_booking_send-contract', {
                ...
                activities_planning: this.activitiesPlanning
            });
            this.is_sent = true;
