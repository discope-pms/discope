...
[$params, $providers] = eQual::announce([
    ...
    'params'        => [
        ...
        'age' => [
            'type'              => 'string',
            'description'       => "The age in which the child is from."
        ],
        'qty' => [
            'type'              => 'integer',
            'description'       => "Quantity of enrollments of the age."
        ]
    ],
    'access'        => [
...
$map_center_ages_enrollments_qty = [];
...
foreach($camps as $camp) {
    foreach($camp['enrollments_ids'] as $enrollment) {
        ...
        $age_key = $enrollment['child_age'];

        if(!isset($map_center_ages_enrollments_qty[$camp['center_id']][$age_key])) {
            $map_center_ages_enrollments_qty[$camp['center_id']][$age_key] = 0;
        }

        $map_center_ages_enrollments_qty[$camp['center_id']][$age_key]++;
    }
}

$center_ids = array_keys($map_center_ages_enrollments_qty);

$centers = Center::search(['id', 'in', $center_ids])
    ->read(['name'])
    ->get();

foreach($map_center_ages_enrollments_qty as $center_id => $map_age_ranges_qty) {
    $center = null;
    ...
    foreach($map_age_ranges_qty as $age => $qty) {
        $result[] = [
            'center'    => $center,
            'age'       => $age,
            'qty'       => $qty
