...
$data = eQual::run('do', 'sale_booking_check-contract', ['id' => $params['id']]);
if(is_array($data) && count($data)) {
...
eQual::run('do', 'sale_booking_check-date-checkin', ['id' => $booking['id']]);

// check booking for due payments
if(!$params['no_payment']) {
    $data = eQual::run('do', 'sale_booking_check-payments', ['id' => $params['id']]);
...
if(!$params['no_composition']) {
    $data = eQual::run('do', 'sale_booking_check-composition', ['id' => $params['id']]);
...
if(!$params['no_rental_unit_cleaned']) {
    $data =  eQual::run('do', 'sale_booking_check-units-ready', ['id' => $booking['id']]);
    if(is_array($data) && count($data)) {
