...
$booking = Booking::id($params['id'])
    ->read([
        ...
        'customer_id' => [
            'rate_class_id',
            'partner_identity_id' => ['lang_id' => ['code']]
        ],
        'fundings_ids' => [
            'type',
            'is_paid',
...
$payment_plan = PaymentPlan::id($params['payment_plan_id'])
    ->read([
        'name',
        'rate_class_id',
        'booking_type_id',
...
if(!$payment_plan) {
    throw new Exception("unknown_payment_plan", QN_ERROR_UNKNOWN_OBJECT);
}
...
$data = eQual::run('do', 'sale_booking_check-contacts', ['id' => $booking['id']]);
...
$data = eQual::run('do', 'sale_booking_check-prices-assignments', ['id' => $booking['id']]);
...
$data = eQual::run('do', 'sale_booking_check-customer', ['id' => $booking['id']]);
...
$dispatch->cancel('lodging.booking.ready', 'sale\booking\Booking', $booking['id']);
...
foreach($booking['contacts_ids'] as $contact) {
    if(strlen($contact['email'] ?? '') > 0) {
        $email = $contact['email'];
        $lang = $booking['customer_id']['partner_identity_id']['lang_id']['code'];
        break;
    }
}
