...
if(count($bookings_ids)) {
    ...
    // raise a notice
    $dispatch->dispatch('lodging.booking.debtor_customer', 'sale\booking\Booking', $params['id'], 'notice', null, [], $links, null, $booking['center_office_id']);
}
else {
    // symetrical removal of the alert (if any)
    $dispatch->cancel('lodging.booking.debtor_customer', 'sale\booking\Booking', $params['id']);
}
