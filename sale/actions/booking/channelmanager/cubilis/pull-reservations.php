...
use sale\booking\channelmanager\BookingLine;
use sale\booking\channelmanager\BookingLineGroup;
use sale\booking\channelmanager\BookingLineGroupAgeRangeAssignment;
use sale\booking\channelmanager\ExtraService;
 ...
 */
$getSanitizedUtf8String = function(string $value): string {
...
try {
    ...
    foreach($properties as $property) {
        ...
        while(!$flag_success) {
            try {
                $data = eQual::run('get', 'sale_booking_channelmanager_cubilis_reservations', ['property_id' => $property['extref_property_id']]);
                $flag_success = true;
            }
            catch(Exception $e) {
                ++$count_attempts;
                    ...
                    // handle creation or modification
                        ...
                        if($booking) {
                            ...
                            if(!in_array($booking['status'], ['confirmed', 'validated'])){
                                ...
                                Booking::id($booking['id'])->update([
                                    'description'           => str_replace("\r\n", "<br />", $getSanitizedUtf8String($reservation['comments'] ?? '')),
                                    'price'                 => $booking_price,
                                    'date_from'             => $date_from,
                                    'date_to'               => $date_to
                                ]);
                                ...
                                if(!empty($reservation['xml'])) {
                                    try {
                                        Booking::id($booking['id'])->update([
                                            'external_data'     => $getSanitizedUtf8String($booking['external_data'] . "\n\n" . $reservation['xml'])
                                        ]);
                                    }
                                    catch(Exception $e) {
                            ...
                            try {
                                $customer = eQual::run('get', 'sale_booking_channelmanager_cubilis_retrieve-customer', [
                            ...
                            $booking = Booking::create([
                                    ...
                                    'external_data'         => $getSanitizedUtf8String($reservation['xml'] ?? ''),
                                    'description'           => str_replace("\r\n", "<br />", $getSanitizedUtf8String($reservation['comments'] ?? '')),
                                    'price'                 => $booking_price,
                                    'date_from'             => $date_from,
                                    'date_to'               => $date_to,
                        ...
                        try {
                            ...
                            foreach($reservation['room_stays'] as $room_stay) {
                                ...
                                try {
                                    $found = false;
                                    $rental_units = eQual::run('get', 'sale_booking_channelmanager_cubilis_retrieve-rentalunit', [
                                    ...
                                    foreach($rental_units as $rental_unit) {
                                        if(!isset($map_assigned_rental_units_ids[$rental_unit['id']]) && $rental_unit['capacity'] >= $nb_pers) {
                                            ...
                                            // also block hierarchy dependencies
                                            if(isset($rental_unit['parent_id']) && $rental_unit['parent_id']) {
                                                $map_assigned_rental_units_ids[$rental_unit['parent_id']] = true;
                                            }
                                            if(isset($rental_unit['children_ids']) && count($rental_unit['children_ids'])) {
                                                foreach($rental_unit['children_ids'] as $child_rental_unit_id) {
                                                    $map_assigned_rental_units_ids[$child_rental_unit_id] = true;
                                                }
                                            }
                                            $found = true;
                                            break;
                                ...
                                $price_lists_ids = PriceList::search([
                                            [
                                                ...
                                                ['date_to', '>=', strtotime('midnight', $date_from)],
                                                // #memo - this does not cover cross-years bookings
                                ...
                                BookingLine::create([
                                        ...
                                        'description'           => $getSanitizedUtf8String($room_stay['room_type']['name'] ?? '')
                                    ]);
                            ...
                            foreach($reservation['extra_services'] as $extra_service) {
                                try {
                                    ...
                                    $booking_line = BookingLine::create([
                                            'booking_id'            => $booking['id'],
                                            'description'           => $getSanitizedUtf8String($extra_service['comments'] ?? ''),
                        ...
                        if(!$city_tax_found) {
                            try {
                                ...
                                $city_tax_price = round($city_tax_total * (1 + $price['vat_rate']), 2);
                                // add a new line relating to the extra service
                                // #memo - we have to do this because we do not receive VAT excl price
                                BookingLine::create([
                                        'booking_id'            => $booking['id'],
                        ...
                        try {
                            ...
                            eQual::run('do', 'sale_booking_channelmanager_cubilis_plan-contingencies-check', ['id' => $booking['id']]);
                        }
                        catch(Exception $e) {
                        ...
                        if($has_overbooking) {
                            ...
                            // (re-)create alert
                            $dispatch->dispatch('lodging.booking.consistency', 'sale\booking\Booking', $booking['id'], 'important', 'sale_booking_check-consistency', ['id' => $booking['id']], [], null, $property['center_office_id']['id']);
                        }
...
foreach($map_property_ack_reservations as $extref_property_id => $reservations_ids) {
    ...
    try {
        eQual::run('do', 'sale_booking_channelmanager_cubilis_ack-reservations', [
...
if($result['warnings'] || $result['errors']) {
    ...
    if($result['errors']) {
        ...
        $message->setTo(constant('EMAIL_ERRORS_RECIPIENT'))
                ...
                ->setBody("<html>
                        <body>
                        <p>Alertes lors de l'execution du script ".__FILE__." au ".date('d/m/Y').' a '.date('H:i').":</p>
                        <pre>".$report."</pre>
                        </body>
                    </html>");
        // queue message
    ...
    foreach($result['center_offices'] as $center_office_id => $center_office) {
        ...
        if(strlen($center_office['email']) && count($center_office['logs'])) {
            ...
            $message->setTo($center_office['email'])
                    ...
                    ->setBody("<html>
                            <body>
                            <p>Action requise suite a la synchronisation Cubilis du ".date('d/m/Y').' a '.date('H:i').":</p>
                            <p>
