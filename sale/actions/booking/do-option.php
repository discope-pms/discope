...
eQual::run('do', 'sale_booking_check-customer-debtor', ['id' => $params['id']]);

// check age ranges assignments
$data = eQual::run('do', 'sale_booking_check-ages-assignments', ['id' => $params['id']]);
if(is_array($data) && count($data)) {
    $errors[] = 'invalid_booking';
}

// check age ranges assignments
$data = eQual::run('do', 'sale_booking_check-mealprefs-assignments', ['id' => $params['id']]);
if(is_array($data) && count($data)) {
    $errors[] = 'invalid_booking';
}

// check rental units assignments
$data = eQual::run('do', 'sale_booking_check-units-assignments', ['id' => $params['id']]);
if(is_array($data) && count($data)) {
    $errors[] = 'invalid_booking';
}

// check list of services
$data = eQual::run('do', 'sale_booking_check-empty', ['id' => $params['id']]);
if(is_array($data) && count($data)) {
    $errors[] = 'empty_booking';
}

// check overbooking
$data = eQual::run('do', 'sale_booking_check-overbooking', ['id' => $params['id']]);
if(is_array($data) && count($data)) {
    $errors[] = 'overbooking_detected';
}

// check sojourns accommodations
$data = eQual::run('do', 'sale_booking_check-sojourns-accomodations', ['id' => $params['id']]);
if(is_array($data) && count($data)) {
