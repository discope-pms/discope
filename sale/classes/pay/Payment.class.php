...
class Payment extends Model {
    ...
    public static function candelete($om, $ids) {
        $payments = $om->read(self::getType(), $ids, [ 'payment_origin', 'payment_method', 'status', 'is_exported', 'is_manual', 'statement_line_id', 'statement_line_id.status' ]);
        ...
        if($payments > 0) {
            foreach($payments as $id => $payment) {
                ...
                if(!$payment['is_manual'] && $payment['payment_origin'] === 'bank' && isset($payment['statement_line_id']) && $payment['statement_line_id.status'] !== 'pending') {
