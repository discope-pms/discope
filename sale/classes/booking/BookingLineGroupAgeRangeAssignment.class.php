...
class BookingLineGroupAgeRangeAssignment extends Model {
    ...
    */
        ...
        if($assignments > 0 && count($assignments)) {
            foreach($assignments as $id => $assignment) {
                ...
                if($qty != $assignment['booking_line_group_id.nb_pers']) {
                    ...
                    $om->update(\sale\booking\BookingLineGroup::getType(), $assignment['booking_line_group_id'], ['nb_pers' => $qty]);
                }
            }
        }
    }
    ...
    public static function ondelete($om, $oids) {
        ...
        if($assignments > 0 && count($assignments)) {
            foreach($assignments as $id => $assignment) {
                $om->update(\sale\booking\BookingLineGroup::getType(), $assignment['booking_line_group_id'], ['nb_pers' => $assignment['booking_line_group_id.nb_pers'] - $assignment['qty']]);
            }
        }
    ...
    public static function cancreate($om, $values, $lang) {
        if(isset($values['booking_line_group_id'])) {
            $groups = $om->read(\sale\booking\BookingLineGroup::getType(), $values['booking_line_group_id'], ['booking_id.status', 'is_extra'], $lang);
            $group = reset($groups);
            if($group['is_extra']) {
