...
class Order extends \equal\orm\Model {
    ...
    /**
     * Assign default customer_id based on the session's center.
     *
     * @param \equal\orm\Collection $self
     * @return void
     * @throws \Exception
     */
    public static function onupdateSessionId($self) {
        // retrieve default customers assigned to centers
        $self->read(['session_id' => ['center_id' => ['id', 'pos_default_customer_id']]]);
        foreach($self as $id => $order) {
            Order::id($id)
                ->update([
                    'center_id'     => $order['session_id']['center_id']['id'],
    ...
    public static function calcName($self) {
        $result = [];
        $self->read(['sequence', 'session_id', 'session_id' => ['cashdesk_id']]);
        foreach($self as $oid => $order) {
    ...
    public static function calcSequence($self) {
        $result = [];
        $self->read(['session_id']);
        foreach($self as $id => $order) {
            $result[$id] = 1;
            $orders_ids = Order::search([
    ...
    public static function calcTotal($self) {
        ...
        foreach($self as $id => $order) {
            ...
            foreach($order['order_lines_ids'] as $line) {
                $result[$id] += $line['total'];
            }
            $result[$id] = round($result[$id], 4);
        }
    ...
    public static function canupdate($om, $ids, $values, $lang) {
        ...
        return parent::canupdate($om, $values);
    }
     ...
     * @param  array                    $lines_ids   Filtered list of identifiers of lines that must generate entries.
    ...
    public static function _generateAccountingEntries($om, $oids, $lines_ids, $lang) {
        ...
        if($orders > 0) {
            ...
            foreach($orders as $oid => $order) {
                ...
                // fetch order lines
                $lines = $om->read('sale\pos\OrderLine', $lines_ids, [
