...
class PosProduct extends Product {
    ...
    public static function getColumns() {
        return [
            ...
            'product_model_id' => [
                'type'              => 'many2one',
                'foreign_object'    => 'sale\catalog\PosProductModel',
    ...
    /**
     * Checks whether an object can be updated, and perform some additional operations if necessary.
    ...
    public static function canupdate($om, $oids, $values, $lang='en') {
        ...
        $users = $om->read(\identity\User::getType(), $user_id, ['centers_ids', 'groups_ids']);
        $user = reset($users);
        ...
        if($products > 0 && count($products)) {
            foreach($products as $id => $product) {
                $centers_ids = array_map(function ($a) {return $a['center_id']; }, (array) $product['product_model_id.groups_ids.center_id']);
    ...
    /**
     * Checks whether the invoice can be deleted.
     *
     * @param  \equal\orm\ObjectManager    $om         ObjectManager instance.
     * @param  array                       $oids       List of objects identifiers.
