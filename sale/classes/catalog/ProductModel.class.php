...
class ProductModel extends Model {
    ...
    public static function onupdateIsPack($self) {
        self::syncProductsFieldValue($self, 'is_pack');
    }

    public static function onupdateHasOwnPrice($self) {
        self::syncProductsFieldValue($self, 'has_own_price');
    }

    public static function onupdateCanSell($self) {
        self::syncProductsFieldValue($self, 'can_sell');
    }

    public static function onupdateFamilyId($self) {
        self::syncProductsFieldValue($self, 'family_id');
    }

    public static function onupdateAllowPriceAdaptation($self) {
        self::syncProductsFieldValue($self, 'allow_price_adaptation');
    }

    /**
     * Syncs model's products value with model value for the given field
     */
    private static function syncProductsFieldValue(Collection $self, string $field) {
        $self->read(['products_ids', $field]);
        foreach($self as $model) {
            if(!empty($model['products_ids'])) {
                Product::search(['id', 'in', $model['products_ids']])
                    ->update([
                        $field => $model[$field]
                    ]);
            }
    ...
    /**
     * Updates related products groups_ids
     */
    public static function onupdateGroupsIds($self) {
        $self->read(['products_ids' => ['groups_ids'], 'groups_ids']);
        foreach($self as $model) {
    ...
    /**
     * Syncs product model capacity and is_accomodation with rental unit
     */
    public static function onupdateRentalUnitId($self) {
        $self->read(['rental_unit_id' => ['capacity', 'is_accomodation']]);
        foreach($self as $id => $model) {
            ProductModel::id($id)
                ->update([
                    'capacity'          => $model['rental_unit_id']['capacity'],
                    'is_accomodation'   => $model['rental_unit_id']['is_accomodation']
                ]);
        }
    }

    /**
     * Sets is_accomodation to false if is_rental_unit is set to false
     */
    public static function onupdateIsRentalUnit($self) {
        $self->read(['is_rental_unit']);
        foreach($self as $id => $model) {
